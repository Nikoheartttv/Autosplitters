...
init
{
    ...
    vars.Helper.TryLoad = (Func<dynamic,bool>)(mono =>
    {
        ...
        {
            case 16: vars.Version = "v0.10.1"; break;
            case 25: vars.Version = "v0.11.5"; break;
            case 26: vars.Version = "v0.11.6"; break;
            case 27: vars.Version = "v0.12.0"; break;
            case 28: vars.Version = "v0.13.0"; break;
            case 29: vars.Version = "v0.13.1"; break;
            case 30: vars.Version = "v0.14.0"; break;
            case 31: vars.Version = "v0.15.0"; break;
            case 32: vars.Version = "v0.15.1"; break;
            case 33: vars.Version = "v0.16.0"; break;
            case 34: vars.Version = "v0.16.1"; break;
            case 35: vars.Version = "v0.17.0"; break;
            case 39: vars.Version = "v0.18.1"; break;
            case 40: vars.Version = "v0.19.0"; break;
            case 41: vars.Version = "v1.0.0"; break;
            case 42: vars.Version = "v1.0.1+"; break;
            default: vars.Version = "v1.0.1+"; break;
        }
    ...
    switch((string)vars.Version)
    {
        ...
        case "v0.11.6": case "v0.11.5": case "v0.10.1":
            ...
            if (current.trueGameover > 0 && current.trueGameover < 5)
            {
                ...
                {
                    // Boss level
                    if (!current.failedLevel && !(current.mistakesCountP1 > 0.0 && settings["Flawless"]))
                        vars.levelCompleted = true;
                }
                else
                {
                    // Normal level
                    if (vars.GetLocalRank(current.rank) >= 10 &&
...
start
{
    if (!settings["IL_Mode"])
    {
        switch((string)vars.Version)
        {
            ...
            case "v0.11.6": case "v0.11.5": case "v0.10.1":
                return !(old.slotOpen && old.transitioningToAnotherScene) && 
                       (current.slotOpen && current.transitioningToAnotherScene);
        }
    }
    else if (current.Scene == "scnGame" && current.Level != "BeansHopper")
    {
        if (current.attemptToLoadTutorial == false &&
            !current.currentLevelPath.Contains("tutorial") &&
            current.GameState == 1) return true;
    }
    else if (current.Scene == "scnGame" && current.Level == "BeansHopper")
    {
                ...
                if (old.Scene == "scnGame" && current.Scene != "scnGame")
                {
                    if (!vars.VisitedLevel.Contains(current.Level))
                    {
                        ...
                        if (current.Level == "BeansHopper")
                            doSplit = !settings["Flawless"] || current.score >= 60;
                        else if (vars.bossLevels.Contains(current.Level))
                            doSplit = vars.levelCompleted;
                        else if (current.Level == "HelpingHands")
                            doSplit = !settings["Flawless"] || vars.GetLocalRank(current.rank) == 17;
                        else
                            doSplit = vars.GetLocalRank(current.rank) >= 10 &&
                                      (!settings["Flawless"] || vars.GetLocalRank(current.rank) == 17);
