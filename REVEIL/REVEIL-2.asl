...
update
{
    ...
    if (old.OverallChapterNum != current.OverallChapterNum) vars.Log("OverallChapterNum: old: " + old.OverallChapterNum.ToString() + ", current: " + current.OverallChapterNum.ToString());
    if (old.Act != current.Act) vars.Log("Act: " + current.Act.ToString());
}
...
split
{
    ...
    if (current.Act == 5 && current.OverallChapterNum == 20 
        ...
        {
            vars.Log("Game Has Finished Split");
            return true;
        }
}
