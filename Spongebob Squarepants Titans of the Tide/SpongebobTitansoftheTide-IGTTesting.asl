...
onStart
{
	...
	vars.LastIGT = 0f;
	vars.IGTStallFrames = 0;
	timer.IsGameTimePaused = vars.Loading || !vars.UseGameTime;
...
update
{
	...
	vars.SetText("TimingMode", "Mode: IGT");
	...
	if (!vars.Loading)
	{
		...
		{
			vars.IGTStallFrames++;
			if (vars.IGTStallFrames > vars.IGTStallThreshold && vars.UseGameTime)
	...
	// Update TimingMode text
	var modeText = vars.UseGameTime ? "Mode: IGT" : "Mode: LRT";
...
gameTime
{
    ...
    if (vars.UseGameTime) igt else null
}
